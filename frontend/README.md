...
Isso gera `frontend/dist/`. O `main.py` (raiz do projeto) já foi ajustado
para servir esses arquivos como estático na mesma origem que responde
`/pergunta` — ou seja, **não precisa rodar `npm run dev` em produção**,
só o Flask:

```bash
# na raiz do projeto, depois do npm run build dentro de frontend/
python main.py
# ou, em produção de verdade: gunicorn main:app (já configurado no Procfile)
```
